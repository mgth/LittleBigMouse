...
namespace HLab.Windows.API
{
        ...
        public enum DeviceCap
    {
        ...
        private static extern bool DDCCIGetCapabilitiesStringLength(
            [In] IntPtr hMonitor, out uint pdwLength);

        [DllImport("gdi32.dll", EntryPoint = "DDCCIGetCapabilitiesString", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool DDCCIGetCapabilitiesString(
            [In] IntPtr hMonitor, StringBuilder pszString, uint dwLength);

        public static string DDCCIGetCapabilitiesString(IntPtr hMonitor)
        {
            if (NativeMethods.DDCCIGetCapabilitiesStringLength(hMonitor, out var len))
            {
                if (len == 0) return null;

                StringBuilder s = new StringBuilder((int)len + 1);
                if (NativeMethods.DDCCIGetCapabilitiesString(hMonitor, s, len))
                {
                    return s.ToString();
                }
            }
