...
//! top of everything, without taking the desktop hostage. Under Wayland a client cannot
//! position its own window at all — which is the whole problem — and the escape the plan
//! names first, XWayland, is already known here to be the wrong one: `lbm-pattern`
//! exists because KWin rescales XWayland surfaces on any screen whose scale differs
//! from the global factor, and a rescaled ruler is not a ruler.
//!
//! So: `zwlr_layer_shell_v1`, which KWin advertises (version 5). A layer surface is
//! given an output, a layer, an anchor and margins — which together are a position —
//! and that is the one protocol that lets a client say where it wants to be.
//!
//! What this spike puts on screen: a band of known size at a known offset on a chosen
//! output, half transparent so the desktop shows through it, with an empty input region
//! so the pointer goes past it to whatever is underneath. It prints what it asked for
//! and what the compositor granted, then leaves on its own.
//!
//! Usage: `lbm-overlay-spike [seconds] [output-name]`, e.g. `lbm-overlay-spike 5 DP-3`.

#[cfg(not(target_os = "linux"))]
fn main() {
    eprintln!("lbm-overlay-spike: Linux/Wayland only for now");
    std::process::exit(2);
}
...
mod linux {
    ...
    /// What the band is asked to be, in the output's own logical pixels.
    const BAND: (i32, i32) = (420, 64);
    /// Where, from the output's top-left corner — the thing Wayland otherwise forbids.
    const AT: (i32, i32) = (160, 120);
