...
public sealed class HisenseControlViewModel : ReactiveObject, IDisposable
{
    ...
    public void SetMonitor(PhysicalMonitor? monitor)
    {
        ...
        if(c is not null){ IpAddress=c.IpAddress; MacAddress=c.MacAddress; ControllerMacAddress=c.ControllerMacAddress; CertificatePath=c.ClientCertificatePath; DeviceUuid=c.DeviceUuid; KeyMacro=c.KeyMacro; Paired=c.HasPairing; ShowSetup=!Paired; Status=Paired ? "Connected to VIDAA." : "Address saved; request a PIN to pair."; if(Paired) BeginVolumeRefresh(monitor.Id); }
    ...
    async Task PairAsync(){if(_service is null||_monitor is null)return;var monitorId=_monitor.Id;await Run(async ct=>{await _service.PairAsync(monitorId,Pin,ct);var c=_service.GetConfiguration(monitorId);IpAddress=c?.IpAddress??IpAddress;Paired=true;ShowSetup=false;Status="Paired and connected over the local network.";},TimeSpan.FromSeconds(45));if(Paired)BeginVolumeRefresh(monitorId);}
